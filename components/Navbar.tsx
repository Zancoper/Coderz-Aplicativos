...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#0f0720]/80 backdrop-blur-md py-3 md:py-4 shadow-xl' : 'bg-transparent py-5 md:py-6'}`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-3">
          {/* Logo Recreation based on attached image */}
          <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg overflow-hidden flex-shrink-0 shadow-lg shadow-blue-900/20">
          ...
          <span className="text-lg md:text-2xl font-black tracking-tight text-white whitespace-nowrap">
            Coderz<span className="text-pink-500">Aplicativoss</span>
          </span>
        </div>
        ...
        <div>
          <a 
            ...
            className="px-3 py-2 sm:px-5 sm:py-2.5 bg-gradient-to-r from-pink-600 to-purple-600 text-white text-[10px] sm:text-xs md:text-sm font-bold rounded-full hover:shadow-lg hover:shadow-pink-500/20 transition-all active:scale-95 whitespace-nowrap"
          >
            Falar no WhatsApp
          </a>
        </div>
